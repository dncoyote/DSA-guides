...
| [Group Anagrams](/dsa/arrayshashing/GroupAnagrams.java)| Create a HashMap that store sorted string as key and `List<String>` of that sorted string as value&rarr;Return the values| O(m n log n) |O(mn)|Create a HashMap that store char[26] count String as key and `List<String>` of that count as value&rarr;Return the values |O(mn)|O(mn)|
...
| [Product Of Array Except Self](/dsa/arrayshashing/ProductOfArrayExceptSelf.java)| use nested loops | O(n<sup>2</sup>) | O(n)|Compute prefix product = product of elements to the left, Compute suffix product = product of elements to the right, multiply both together | O(n) |O(1) |
